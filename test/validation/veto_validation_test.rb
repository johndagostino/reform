...
class VetoValidationTest < MiniTest::Spec
  ...
  class SessionForm < Reform::Form
    ...
    validation :email, if: :default do
      ...
      validates :email, exact_length: 3
    end

    validation :nested, if: :default do
      validates :password, presence: true, exact_length: 1
    end

    validation :confirm, if: :default, after: :email do
      validates :confirm_password, exact_length: 2
    end
  end
